...
static lv_img_dsc_t *photo_thumb_img_dsc = NULL;
static lv_point_t *photo_thumb_point = NULL;
static void photo_thumb_img_dsc_alloc(void)
{
        lv_point_t area = {0, 0};
        if (photo_thumb_img_dsc == NULL)
        {
                photo_thumb_img_dsc = lv_img_buf_alloc(PHOTO_WIDTH, PHOTO_HIGHT, LV_IMG_CF_TRUE_COLOR);
                photo_thumb_img_dsc->header.reserved = 0x03;
        }
        if (photo_thumb_point == NULL)
        {
                photo_thumb_point = lv_mem_alloc(sizeof(lv_point_t));
                lv_memcpy_small(photo_thumb_point, &area, sizeof(lv_point_t));
        }
}
static void photo_thumb_img_dse_destroy(void)
{
        if (photo_thumb_img_dsc != NULL)
        {
                lv_img_buf_free(photo_thumb_img_dsc);
                photo_thumb_img_dsc = NULL;
        }
        if (photo_thumb_point != NULL)
        {
                lv_mem_free(photo_thumb_point);
                photo_thumb_point = NULL;
        }
}

static lv_obj_t *photo_msgbox_create(const char *title, const char *msg, lv_event_cb_t cancel_cb, lv_event_cb_t confirm_cb)
{
        lv_obj_t *parent = lv_common_img_btn_create(sat_cur_layout_screen_get(), photo_obj_id_msgbox, 0, 0, 1024, 600,
...
static void photo_obj_cancel_click(lv_event_t *e)
{
        sat_layout_goto(playback, LV_SCR_LOAD_ANIM_FADE_IN, SAT_VOID);
}

static void photo_info_obj_display(void)
{
        lv_obj_t *parent = lv_obj_get_child_form_id(sat_cur_layout_screen_get(), photo_obj_id_top);
        if (parent == NULL)
        {
                SAT_DEBUG("lv_obj_t* parent = lv_obj_get_child_form_id(sat_cur_layout_screen_get(), photo_obj_id_top);");
                return;
        }
...
{
        lv_obj_t *parent = lv_obj_get_child_form_id(sat_cur_layout_screen_get(), photo_obj_id_buttom);
        if (parent == NULL)
        {
                SAT_DEBUG("lv_obj_t* parent = lv_obj_get_child_form_id(sat_cur_layout_screen_get(), photo_obj_id_buttom);");
                return;
        }
...
static void photo_thumb_refresh_display_callback(void)
{

        lv_obj_t *parent = lv_obj_get_child_form_id(sat_cur_layout_screen_get(), photo_obj_id_thumb);
        if (parent == NULL)
        {
                SAT_DEBUG(" lv_obj_t *parent = lv_obj_get_child_form_id(sat_cur_layout_screen_get(), photo_obj_id_thumb);");
                return;
        }
...
static bool photo_thumb_media_display_callback(const char *data, int x, int y, int w, int h)
{
        if ((photo_thumb_img_dsc != NULL) && (photo_thumb_point != NULL) && (photo_thumb_point->x == x) && (photo_thumb_point->y == y))
        {
                lv_memcpy_small((uint8_t *)photo_thumb_img_dsc->data, data, w * h * 3);
                return true;
        }
        SAT_DEBUG("photo display failed(%d,%d,%d,%d)\n", x, y, w, h);
        return false;
}
static void photo_thumb_decode_all_display(void)
{
        /*显示加载的图像*/
...
static void photo_media_thumb_obj_click(lv_event_t *e)
{
        lv_obj_t *obj = lv_obj_get_child_form_id(sat_cur_layout_screen_get(), photo_obj_id_top);
        if (obj == NULL)
        {
                SAT_DEBUG("lv_obj_t* obj = lv_obj_get_child_form_id(sat_cur_layout_screen_get(),photo_obj_id_top);");
                return;
        }
...
static void photo_msgbox_del_cancel_click(lv_event_t *e)
{
        lv_obj_t *obj = lv_obj_get_child_form_id(sat_cur_layout_screen_get(), photo_obj_id_msgbox);
        if (obj == NULL)
        {
                return;
        }
...
static void photo_del_msgbox_confirm_click(lv_event_t *e)
{
        lv_obj_t *obj = lv_obj_get_child_form_id(sat_cur_layout_screen_get(), photo_obj_id_msgbox);
        if (obj == NULL)
        {
        ...
        {
                sat_layout_goto(video, LV_SCR_LOAD_ANIM_FADE_IN, SAT_VOID);
        }
        else
        {
...
static void photo_obj_del_click(lv_event_t *e)
{
        photo_msgbox_create(layout_setting_storage_space_language_get(SETTING_STORAGE_LANG_ID_DELETE),
...
static void sat_layout_enter(photo)
{
        ...
        ***********************************************/
        {
                photo_thumb_img_dsc_alloc();
                thumb_display_refresh_register(photo_thumb_refresh_display_callback);
                sd_state_channge_callback_register(photo_sd_state_change_callback);
        }
        /***********************************************
         ** 作者: leo.liu
         ** 日期: 2023-2-2 13:42:25
         ...
         ***********************************************/
        {
                lv_common_img_btn_create(sat_cur_layout_screen_get(), photo_obj_id_thumb, (int)photo_thumb_point->x, (int)photo_thumb_point->y, PHOTO_WIDTH, PHOTO_HIGHT,
                                         photo_media_thumb_obj_click, true, LV_OPA_TRANSP, 0x242526, LV_OPA_TRANSP, 0x242526,
                                         0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
                                         0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
                                         NULL, LV_OPA_TRANSP, 0x00a8ff, LV_ALIGN_CENTER);
        }
        ...
        ***********************************************/
        {
                lv_obj_t *parent = lv_common_img_btn_create(sat_cur_layout_screen_get(), photo_obj_id_top, 0, 0, 1024, 80,
                        ...
                        ***********************************************/
                        {
                                lv_common_img_btn_create(parent, 1, 35, 15, 48, 48,
                                                         photo_obj_cancel_click, true, LV_OPA_TRANSP, 0, LV_OPA_TRANSP, 0x808080,
                                                         0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
                                                         0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
                         ...
                         ***********************************************/
                        {
                                lv_common_img_btn_create(parent, 2, 930, 0, 80, 80,
                                                         photo_obj_del_click, true, LV_OPA_TRANSP, 0, LV_OPA_TRANSP, 0x808080,
                                                         0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
                                                         0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
        ...
        ***********************************************/
        {
                lv_obj_t *obj1 = lv_common_img_btn_create(sat_cur_layout_screen_get(), photo_obj_id_left, 24, 300, 80, 80,
                                                          photo_obj_left_click, true, LV_OPA_TRANSP, 0, LV_OPA_TRANSP, 0x808080,
                                                          0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
                                                          0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
                ...
                lv_obj_t *obj2 = lv_common_img_btn_create(sat_cur_layout_screen_get(), photo_obj_id_right, 919, 300, 80, 80,
                                                          photo_obj_right_click, true, LV_OPA_TRANSP, 0, LV_OPA_TRANSP, 0x808080,
                                                          0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
                                                          0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
         ...
         ***********************************************/
        {
                lv_obj_t *parent = lv_common_img_btn_create(sat_cur_layout_screen_get(), photo_obj_id_buttom, 0, 488, 1024, 112,
                 ...
                 ***********************************************/
                {
                        lv_common_text_create(parent, 0, 24, 63, 300, 33,
                                              NULL, LV_OPA_TRANSP, 0, LV_OPA_TRANSP, 0,
                                              0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
                                              0, 0, LV_BORDER_SIDE_NONE, LV_OPA_TRANSP, 0,
