...
struct vpss_af_stat_info {
   unsigned int  af_statics[5];            //ç»Ÿè®¡ç»“æœ
};
...
struct vpss_af_attr {
	...
	unsigned short   af_th;       //[0, 128],     af_thÎª·§Öµ£¬´óÓÚ¸ÃÖµµÄ¸ßÍ¨ÂË²¨½á¹û½«±»½ØÈ¡¡£ÆäÖĞ¸ßÍ¨ÂË²¨½á¹ûÀ´×ÔÓÚsharpÄ£¿éÖĞµÄ¸ßÆµÈñ»¯½á¹û¡£
...
struct vpss_isp_ae_run_info {
    ...
    unsigned char current_calc_avg_compensation_lumi;	//¾­¹ıÆØ¹â²¹³¥ºóµÄÁÁ¶ÈÖµ

    unsigned char current_darked_flag;                 	//°×ÌìºÚÒ¹µÄ±ê¼Ç

    long  current_a_gain;				//Ä£ÄâÔöÒæµÄÖµ
    long  current_d_gain;				//Êı×ÖÔöÒæµÄÖµ
    long  current_isp_d_gain;			//ispÊı×ÖÔöÒæµÄÖµ
    long  current_exp_time;				//ÆØ¹âÊ±¼äµÄÖµ

    unsigned long  current_a_gain_step;	//ÏÖÔÚµÄÄ£ÄâÔöÒæµÄ²½³¤
    unsigned long  current_d_gain_step; //Êı×ÖÔöÒæµÄ²½³¤
    unsigned long  current_isp_d_gain_step;	//ispÊı×ÖÔöÒæµÄ²½³¤
...
struct vpss_isp_ae_init_info {
    ...
    long  d_gain;				//Êı×ÖÔöÒæµÄÖµ
    long  isp_d_gain;			//ispÊı×ÖÔöÒæµÄÖµ
    long  exp_time;				//ÆØ¹âÊ±¼äµÄÖµ
};

struct vpss_isp_ae_attr {
	unsigned int exp_time_max;			//ÆØ¹âÊ±¼äµÄ×î´óÖµ
    unsigned int exp_time_min;			//ÆØ¹âÊ±¼äµÄ×îĞ¡Öµ
    unsigned int d_gain_max;      		//Êı×ÖÔöÒæµÄ×î´óÖµ
    unsigned int d_gain_min;     		//Êı×ÖÔöÒæµÄ×îĞ¡Öµ
    unsigned int isp_d_gain_min;  		//ispÊı×ÖÔöÒæµÄ×îĞ¡
    unsigned int isp_d_gain_max;  		//ispÊı×ÖÔöÒæµÄ×î´óÖµ
    unsigned int a_gain_max;     		//Ä£ÄâÔöÒæµÄ×î´óÖµ
    unsigned int a_gain_min;      		//Ä£ÄâÔöÒæµÄ×îĞ¡Öµ
    ...
    unsigned int BC_rate_min;			//Ö±·½Í¼°µÇøÏñËØ×îĞ¡±ÈÂÊ
};
...
struct vpss_isp_awb_stat_info {
	//ÔÚ°×Æ½ºâÍ³¼Æ²ÎÊı·¶Î§ÄÚµÄ°×Æ½ºâÍ³¼Æ½á¹û
    ...
    unsigned long  total_cnt[10];	//ÔÚ°×Æ½ºâÍ³¼Æ²ÎÊı·¶Î§ÄÚµÄÏñËØÊıÁ¿Öµ
    ...
    unsigned short  current_colortemp_index;   //»·¾³É«ÎÂ±ê¼Ç£¬ÊÇ²ÎÊıËæ»·¾³±ä»¯µÄÉ«ÎÂÖ¸±ê  
    unsigned short  colortemp_stable_cnt[10];  //Ã¿Ò»ÖÖÉ«ÎÂÎÈ¶¨µÄÖ¡Êı¼ÆÊı   
};
...
struct vpss_isp_wdr {
    ...
    unsigned short   hdr_cnoise_suppress_slop;   //æŠ‘åˆ¶æ–œç‡
...
struct vpss_isp_3d_nr {
	...
	unsigned short		updata_ref;			    //¸üĞÂy/uv,default:1
	...
	unsigned int		md_th;			        //[0, 65535]  ÔË¶¯¼ì²âãĞÖµ[0-127],default:0
};
...
struct vpss_isp_awb_attr {
    ...
    //awbÈí¼ş²¿·ÖĞèÒªÉèÖÃµÄ²ÎÊı
    unsigned short auto_wb_step;                //°×Æ½ºâ²½³¤¼ÆËã
    unsigned short total_cnt_thresh;            //ÏñËØ¸öÊıãĞÖµ
    unsigned short colortemp_stable_cnt_thresh; //ÎÈ¶¨Ö¡Êı£¬¶àÉÙÖ¡Ò»ÑùÈÏÎª»·¾³É«ÎÂ¸Ä±ä
